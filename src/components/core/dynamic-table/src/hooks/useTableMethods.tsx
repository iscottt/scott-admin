...
export const useTableMethods = ({ state, props, emit }: UseTableMethodsContext) => {
  ...
  const fetchData = async (params = {}) => {
    ...
    if (
      ...
      const _pagination: any = unref(paginationRef)!;
      // 是否启用了分页
      ...
      const data = await props?.dataRequest?.(queryParams).finally(() => (loadingRef.value = false));
      if (data?.pagination) {
        const { pageSize, current, total } = data.pagination;

        if (enablePagination && _pagination?.current) {
          // 有分页时,删除当前页最后一条数据时 往前一页查询
          if (data?.rows.length === 0 && total > 0 && current > 1) {
            _pagination.current--;
            return reload();
          }
        }

        Object.assign(unref(paginationRef as any), {
          current: ~~current,
          pageSize: ~~pageSize,
          total: ~~total,
        });
  ...
  const reload = async (resetPageIndex = false) => {
    const pagination: any = unref(paginationRef);
  ...
  const handleTableChange: OnChangeCallbackParams = (...rest) => {
    ...
    if (Object.keys(pagination).length) {
      Object.assign(unref(paginationRef as any), pagination);
    }
    fetchData();
    emit('change', ...rest);
  };
